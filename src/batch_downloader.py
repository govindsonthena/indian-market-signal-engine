...
from src.config import BATCH_SIZE, DOWNLOAD_TIMEOUT, HISTORY_PERIOD, MIN_DATA_ROWS, RETRY_ATTEMPTS, RETRY_DELAY_SECONDS
...
def download_batch(symbols: list[str], period: str = HISTORY_PERIOD, timeout: float = DOWNLOAD_TIMEOUT,
                  retries: int = RETRY_ATTEMPTS, retry_delay: float = RETRY_DELAY_SECONDS) -> dict[str, pd.DataFrame]:
    ...
    for attempt in range(retries):
        try:
            data = yf.download(ticker_string, period=period, progress=False, timeout=timeout)
            ...
            logger.warning("Batch attempt %s/%s failed for %s tickers: %s", attempt + 1, retries, len(symbols), error)
            if attempt < retries - 1:
                time.sleep(retry_delay)

    message = f"Batch download failed after {retries} attempts for {len(symbols)} tickers"
    logger.error(message)
    raise BatchDownloadError(message)
...
def batch_download_universe(symbols: list[str], batch_size: int = BATCH_SIZE, period: str = HISTORY_PERIOD,
                            ...
                            retry_delay: float = RETRY_DELAY_SECONDS) -> BatchDownloadResult:
    ...
    for batch_num, batch in enumerate(batches, 1):
        ...
        try:
            batch_data = download_batch(batch, period, timeout, retries, retry_delay)
        except BatchDownloadError as error:
            for symbol in batch:
                result.download_errors[symbol] = str(error)
            continue
